...
void UHorrorInteractComponent::BeginPlay()
{
    Super::BeginPlay();
    check(GetWorld);
    GetWorld()->GetTimerManager().SetTimer(TraceTimerHandle, this, &URPGInteractComponent::TraceForward_Implementation, TraceRate, true);
}
...
void UHorrorInteractComponent::TraceForward_Implementation()
{
    ...
    if (FocusedActor != Interactiable)
    {
        ...
        IRPGInteractiveInterface* Interface = Cast<IHorrorInteractiveInterface>(FocusedActor);
        if (Interface)
        {
...
void UHorrorInteractComponent::EndFocus()
{
    ...
    IRPGInteractiveInterface* Interface = Cast<IRPGInteractiveInterface>(FocusedActor);
